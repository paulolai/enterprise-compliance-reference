...
async function performWeirdAction(page: Page): Promise<string> {
  const actions = [
    ...
    async () => {
      ...
      if (count > 0) {
        const randomBtn = buttons.nth(Math.floor(Math.random() * count));
        const isEnabled = await randomBtn.isEnabled().catch(() => false);
        if (isEnabled) {
...
async function runWeightedWalk(page: Page) {
  ...
  while (Date.now() - start < TEST_DURATION_MS) {
    try {
      ...
      errors++;
      console.log(`⚠️ Action failed (error #${errors}): ${error}`);
  ...
  console.log(`\n✅ Walk complete: ${actions} actions, ${errors} errors`);
}
